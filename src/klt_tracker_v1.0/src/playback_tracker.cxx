...
//added in by Matthew
void aa2Quat(double* axis, double angle, double* q);
void quatMultiply(double* x, double* y, double* q);
							...
							const double* KK, const double* Ki, double H[9], int sequence, int imuAid)
{
	...
	//Their IMU Method, the velocitySwitch Method, Axis Angle
	//if(/*(imuAid == 2) || (imuAid == 3) || (imuAid == 4)*/)
	...
	double q[4] = {1, 0, 0, 0};	//identity quaternion
	int deltaT = 2;
	int tt, n = (int)vdata.size();	//number of imu samples

	if (index >= n-1) {				//if we ran out of imu samples, set the "identity" homography and exit
		H[0] = H[4] = H[8] = 1.0;
	...
	//cout << "ptime = " << ptime << "  ctime = " << ctime << endl;
	for (tt = ptime; tt < ctime; ) 
	{
		...
		//cout << "tt = " << tt << "  ti = " << ti << "  tj = " << tj << endl; 

		if (tt < ti) { 
			a = ((ti - tt) < 30) ? 1.0 : 0.0;		//is the 30 here camera frame rate?
			//if the imu time is less than 30 ms more than current iteration time
				//use the current imu data
			//if not
				//use the next imu index data

			//this is a weird case that should never happen, but if it does the tt will catch up with the imu time
		}
		else if (tt >= ti && tt < tj) {	
			a = (double)(tj - tt)/(double)(tj - ti);		//won't this always be 1? or is it an interpolation?
		}
		else if (tt >= tj) {
			if (index++ > n-1) break;
			else continue;
		...
		double w[3];
			//if a=1 use the current time imu data. Else use the next index data
		...
		//Quaternion Integration Using Axis Angle Approach
		if(imuAid == 4)
		{
			//obtain axis angle representation
			double wMag = sqrt(w[0]*w[0] + w[1]*w[1] + w[2]*w[2]);
			double axis[3] = {w[0]/wMag, w[1]/wMag, w[2]/wMag};
			double angle = wMag * 0.001*deltaT;

			//convert to quaternion representation
			double dq[4]; 
			aa2Quat(axis, angle, dq);

			//aggregate the quaternions with multiplication (TODO double check this is the correct direction)
			double q_temp[4];
			q_temp[0] = q[0];
			q_temp[1] = q[1];
			q_temp[2] = q[2];
			q_temp[3] = q[3];
			quatMultiply(dq, q_temp, q);				//q = dq * q_temp
		}
		else
		{
	...
	//velocitySwitch method (detect velocity based on the quaternion rotation)
	if( (imuAid == 3) && (q[0] > 0.99999) )
	{
		if(q[0] > 0.99999)
		{
			//Ri[0] = Ri[4] = Ri[8] = 1.0;
			//Ri[1] = Ri[2] = Ri[3] = Ri[5] = Ri[6] = Ri[7] = 0.0;
			H[0] = H[4] = H[8] = 1.0;
			H[1] = H[2] = H[3] = H[5] = H[6] = H[7] = 0.0;

			//cout << "Low Motion Detected!" << endl;			
	...
	//If this is the EuRoC Dataset
		if( ( (sequence >=1)&&(sequence <= 5) ) ||
			( (sequence >=8)&&(sequence <=13) )  )
		{
			//Here insert the similarity transform 
			//note they do it backwards so this is actually the inverse from the other matrix so its cam_imu
			double R_cam_imu[9] = {0.0148655429818, -0.999880929698, 0.00414029679422,
								   0.999557249008,  0.0149672133247, 0.025715529948,
								   -0.0257744366974, 0.00375618835797, 0.999660727178};

			double R_imu_cam[9];
			transpose_mat_3x3(R_cam_imu, R_imu_cam);
			double newRi[9];

			mul_mat_3x3(R_cam_imu, Ri, R_imu_cam, newRi);
...
// q = aa2Quat(axis, angle) ... but pointers ...
void aa2Quat(double* axis, double angle, double* q)
{
	if(q == NULL)
		return;
	
	double sinAng_2 = sin(angle/2);		
...
void quatMultiply(double* x, double* y, double* q)
{
	...
	q[0] = x[0]*y[0] - x[1]*y[1] - x[2]*y[2] - x[3]*y[3]; 
	q[1] = x[0]*y[1] + x[1]*y[0] + x[2]*y[3] - x[3]*y[2];
	q[2] = x[0]*y[2] + x[2]*y[0] + x[3]*y[1] - x[1]*y[3];
	q[3] = x[0]*y[3] + x[3]*y[0] + x[1]*y[2] - x[2]*y[1];
}
