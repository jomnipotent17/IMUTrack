...
//Matthew Added in some Global Variables
int sequence = 6;	//1-13 for the 13 sequences
int imuAid = 1;		//1=noIMU, 2=theirIMU, 3=newIMUMethod
char* newTrackLogName;	//Because I'm lazy
//double imuTimeStep = 0.05;	//default to EuRoC
...
int main(int argc, char* argv[])
{
	...
	//Matthew
	//make the log file name from the inputs
	string logFileName = logFileNameMaker();
	//newTrackLogName = logFileName.c_str();

	//string str ("Please split this sentence into tokens");

  	char * newTrackLogName = new char [logFileName.length()+1];
  	std::strcpy (newTrackLogName, logFileName.c_str());
	...
	while(b_run)
	{
		...
		if (b_imu && ptime != 0) {
			if (!compute_homography_IMU(ptime, ctime, imu_list_name, imu_shape_name, 
					cam->GetKK(KK), cam->GetKi(Ki), H, sequence, imuAid)   ) break;
		}
...
string logFileNameMaker()
{
	...
	switch(imuAid) {
		...
		case 3: 
		{
			model = "velocitySwitch";  
			break;
		}
		case 4: 
		{
			model = "axisAngleIntegration";  
			break;
		}
		default: 
		{
...
bool read_input_arg(int argc, char* argv[])
{
	for (int i=1; i < argc; i++) 
	{
		if      (strncmp(argv[i], "-log",   4) == 0)  b_log = true;
			...
			if ((i+1) >= argc) return false;
			sequence = atoi(argv[i+1]); i++;
			cout << "sequence: " << sequence << endl;
		}
		else if (strncmp(argv[i], "-model", 6) == 0) {
			if ((i+1) >= argc) return false;
			imuAid = atoi(argv[i+1]); i++;
			if(imuAid != 1)
			{
				b_imu = true;
			}
			cout << "imuAid type: " << imuAid << endl;
		}
		else {
			printf("Invalid switch: %s\n\n",argv[i]);
			printf("usage: %s [-f config] [-log] [-gpu] [-stop] [-imu time_ms] [-n num] [-t size] [-disp scale]\n\n", argv[0]);
			//printf("usage: %s [-f config] [-log] [-gpu] [-stop] [-imu time_ms] [-n num] [-t size] [-disp scale] [-seq value] [-model value]\n\n", argv[0]);
			return false;
		}
	}
